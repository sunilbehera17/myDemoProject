...
public class WebUI {
    ...
    public static void openWebsite(String URL) {
        ...
        LogUtils.info("Open the Website with URL: " + URL);

        if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Open the Website with URL: " + URL);
        }
    ...
    public static void setText(String fieldName,By by, String value) {
        waitForElementVisible(by).sendKeys(value);
        LogUtils.info("Set text " + value + " on " + fieldName/*by.toString()*/);

        if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Set text " + value + " on " + fieldName);
        }
    ...
    public static void clearText(String fieldName, By by) {
        waitForElementVisible(by).clear();
        LogUtils.info("Clear the text in the textbox. " + fieldName/*by.toString()*/);

        if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Clear the text in the textbox. " + fieldName/*by*/);
    ...
    public static void clearTextCtrlA(By by) {
        ...
        LogUtils.info("Clear the text in the textbox. " + by.toString());
        if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Clear the text in the textbox. " + by);
        }
        //AllureManager.saveTextLog("Clear text in textbox");
        addScreenshotToReport(Thread.currentThread().getStackTrace()[1].getMethodName() + "_" + DateUtils.getCurrentDateTime());
    ...
    public static void clickElement(String fieldName, By by) {
        waitForElementClickable(fieldName, by).click();
        LogUtils.info("Clicked on the " + fieldName/*by.toString()*/);

        if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Clicked on the " + fieldName);
        }
    ...
    public static void clickElement(String fieldName,By by, int timeout) {
        //smartWait();
        waitForElementClickable(fieldName, by, timeout).click();
        LogUtils.info("Clicked on the element " + fieldName +" "+ by.toString());
...
//    }
     ...
     */
    public static WebElement waitForElementClickable(String fieldName,By by, long timeOut) {
        try {
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(timeOut), Duration.ofMillis(500));
            return wait.until(ExpectedConditions.elementToBeClickable(by));
        } catch (Throwable error) {
            LogUtils.error("❌ Timeout waiting for the element ready to click. " + fieldName +" "+by.toString());
            Assert.fail("❌ Timeout waiting for the element ready to click. " + fieldName +" "+by.toString());
        }
        return null;
    }
    
    public static boolean waitForElement(By by, long timeOut) {
    	try {
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(FrameworkConstants.WAIT_EXPLICIT), Duration.ofMillis(500));
            wait.until(ExpectedConditions.elementToBeClickable(by));
            return true;
     ...
     * @return a WebElement object ready to CLICK
     */
    public static WebElement waitForElementClickable(String fieldName,By by) {
        try {
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(FrameworkConstants.WAIT_EXPLICIT), Duration.ofMillis(500));
            return wait.until(ExpectedConditions.elementToBeClickable(by));
        } catch (Throwable error) {
